...
namespace Ciribob.DCS.SimpleRadio.Standalone.Client.Overlord.Intents
{
    class ReadytoTaxi
    {
        ...
        public static async Task<string> Process(string airbaseName, Player sender)
        {
            ...
            try
            {
                airfield = Airfields.First(x => x.Name == airbaseName);
            }
            ...
            try
            {
                taxiInstructions = new GroundController(airfield).GetTaxiInstructions(sender.Position);
        ...
        private static string ConvertTaxiInstructionsToSSML(TaxiInstructions taxiInstructions)
        {
            ...
            {
                spokenInstructions += $" {Random(viaVariants)} {string.Join(" <break time=\"60ms\" /> ", taxiInstructions.TaxiwayNames)}";
            }
